...
void setTimerTime1() {
  if (server.hasArg("min") && server.hasArg("sec")) {
    ...
    min1 = minStr.toInt();
    sec1 = secStr.toInt();
    Serial.println("타이머 실행 체크 1 - " + minStr + "분 " + secStr + "초");
...
void setTimerTime2() {
  if (server.hasArg("min") && server.hasArg("sec")) {
    ...
    Serial.println("타이머 실행 체크 2 - " + minStr + "분 " + secStr + "초");
...
void setTimerTime3() {
  if (server.hasArg("min") && server.hasArg("sec")) {
    ...
    Serial.println("타이머 실행 체크 3 - " + minStr + "분 " + secStr + "초");
  }
  server.send(200, "text/html", index_html);
}
...
void cleaningStart(){
  if(!digitalRead(18)){ // 18(10)번 포트가 LOW라면 => 청소Sign에서 LOW로 변경이 제대로 됬다면
    digitalWrite(18, HIGH);
    int stopTime1 = (min1*60 + sec1)*1000; // 출발전 대기시간
    delay(stopTime1);
    Serial.println("청소시작");
    digitalWrite(19, HIGH); // 19(9)번포트를 HIGH로 설정하고 청소를 시작함
    delay(100);
    cleaningLoading();
  }else{
    Serial.println("오류 발생1, 처음부터 다시 시작");
    delay(1000);
    cleaningSign();
  }
}
...
void cleaningLoading(){
  ...
  if(digitalRead(19)){
    /**
       청소 중 코드
    **/
    int stopTime2 = (min2*60 + sec2)*1000; // 청소에 걸리는 시간
    delay(stopTime2);
    digitalWrite(19, LOW); // 청소 종료 신호
    delay(100);
    cleaningEnd();
  }else{
    Serial.println("오류 발생2, 처음부터 다시 시작");
    cleaningSign();
  }
}
...
void cleaningEnd(){ / 19(9)번 포트가 LOW로 변경됬다면
  if(!digitalRead(19)){
  ...
  }else{ // 19(9)번 포트가 HIGH라면(청소가 끝나지 않았다면)

    /**
       홈 IR 신호 발생
    **/

    int stopTime3 = (min3*60 + sec3)*1000; // 추가 대기시간
    delay(stopTime3);
    retryHomeIR();
...
void loop() {
  ...
  if(!digitalRead(18)){
    Serial.println("18(10)번 포트가 LOW입니다");
  }else{
    Serial.println("18(10)번 포트가 HIGH입니다(체크용)");
  }
  
  if(!digitalRead(19)){
    Serial.println("19(9)번 포트가 LOW입니다");
  }else{
    Serial.println("19(9)번 포트가 HIGH입니다(체크용)");
  }
